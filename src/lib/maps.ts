...
export const getMapsImage = async (location: string): Promise<string> => {
	const url = encodeURI(`https://maps.googleapis.com/maps/api/staticmap?center=${location}&zoom=17&size=400x300&scale=2&markers=size:mid|color:red|${location}&key=${import.meta.env.GOOGLE_MAPS_API_KEY}`);
...
async function encodeBase64Hash(key: Uint8Array, data: string): Promise<string> {
...
async function signMapsStaticURL(path: string, secret: string): Promise<string> {
	const uri = new URL(path);
	const safeSecret = decodeBase64Hash(removeWebSafe(secret));
	const hashedSignature = makeWebSafe(await encodeBase64Hash(safeSecret, uri.pathname + uri.search));
